...
export default async function payIn(
	_amount,
	...
	balances_msgSender =
		parseInt(balances_msgSender.integer, 10) + parseInt(amount.integer, 10);

	balances_msgSender = generalise(balances_msgSender);

	let total = generalise(
		parseInt(unpaid_msgSender.amount.integer, 10) +
			parseInt(unpaid_msgSender.tax.integer, 10) -
			parseInt(unpaid_msgSender.amountPaid.integer, 10)
	);

	if (parseInt(balances_msgSender.integer, 10) >= parseInt(total.integer, 10)) {
		unpaid_msgSender.amount = 0;

		unpaid_msgSender.tax = 0;

		unpaid_msgSender.amountPaid = 0;

		balances_msgSender =
			parseInt(balances_msgSender.integer, 10) - parseInt(total.integer, 10);
	} else {
		unpaid_msgSender.amountPaid =
			parseInt(unpaid_msgSender.amountPaid.integer, 10) +
			parseInt(balances_msgSender.integer, 10);

		balances_msgSender = 0;
	}
	...
	const res = await generateProof("payIn", allInputs);
